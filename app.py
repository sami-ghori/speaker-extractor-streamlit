...
def extract_speaker_clips(video_path, output_folder="clips"):
    ...
    for i, (start, end) in enumerate(merged):
        ...
        if duration < 5:
            continue
        # Ensure clip is 20–30 seconds long
        if duration < 20:
            end = min(start + 20, clip.duration)
        elif duration > 30:
            end = start + 30
        subclip = clip.subclip(start, end)
        audio_array = subclip.audio.to_soundarray(fps=44100)
        reduced = nr.reduce_noise(y=audio_array.T, sr=44100)
        reduced = np.clip(reduced.T, -1, 1)
        audioclip = AudioArrayClip(reduced, fps=44100)
        subclip = subclip.set_audio(audioclip)
...
uploaded_video = st.file_uploader("Upload a video", type=["mp4", "mov", "avi", "mpeg"])
if uploaded_video:
